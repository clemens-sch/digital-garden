...
Special methods in Python, like **init**, for customizing object behavior
...
        self.totalName = f"{firstname} {lastname}"
        self.birDat = social[4:]
```


`__str__` - used for a .ToString() Method
...
        
    def __str__(self):
        return f"{self.totalName};{self.birDat}"
```
